...
| 선행 | B3, D1 |
| 완료 | main `6d793b6` (2026-09-22). 테스트 30개 추가(전체 149 통과). 흐름은 SEQ-4대로 상태 확인 → 대조 → 매도 → **잔고 재조회** → 매수(점수 순) → 지수 비중 조정이고, 핵심은 예산을 판단 시점 평가액이 아니라 매도 대금이 들어온 실제 계좌에서 계산하는 것이다. 거래정지·부분 체결로 못 판 수량은 보류(held)로 남기고 `retry_held_sells`가 매일 다시 시도한다(같은 판단에서 다시 팔 수 있게 멱등 키에 시도 번호 `#1`을 붙인다). 대조는 보유 수량과 예수금을 본다. 예수금은 "마지막으로 맞은 대조의 계좌 현금 + 그 뒤 우리 체결"로 설명돼야 하고, 기준점은 시각이 아니라 체결 번호다(같은 초에 일어난 대조와 체결은 시각으로 앞뒤를 가릴 수 없다 → `reconciliation.last_fill_id` 추가, 마이그레이션 `b71c04e5aa10`). 도구 5개: reconcile·reconcile_accept·positions와 **명령줄 전용 decide·execute**(API-001에 없다. 스케줄(E) 전까지 손으로 돌리기 위한 것). 판단 예산도 이제 실제 계좌에서 온다. 지수 ETF는 069500 상수(StrategyConfig에 칸이 없다). 실호출 확인: `qbot reconcile` → ok(예수금 1,000만원), `qbot positions`, `qbot status`. 실데이터 재현 점검(2023-04-28)은 0종목이었고 원인이 코드가 아니라 공시 부족임을 확인했다(재무 1,502행 중 op_annual_prev 13행뿐 → OPG·OPG_Q가 NaN, 가격·EP·SP·ROE·VOL60·TURN20은 2,981종목 정상). 자체 점검 1건: 종목 코드를 못 찾는 보유 행이 있으면 정렬에서 죽는데 하필 매도는 하고 매수 전이었다. 리뷰 10건 반영(치명 2): **불일치 다음 대조가 "지금 계좌"에 다시 닻을 내려 사람 손 없이 ok가 되던 문제**(기준점을 마지막으로 맞은 대조로), **execute가 정지·주문멈춤을 보지 않고 관문이 전부 거부해도 done으로 닫아 그 달 리밸런싱이 통째로 사라지던 문제**(done은 다시 못 돌린다 → 거부·오류가 있으면 partial). 높음 3: 보류 처리가 끝난 주문 행을 덮어써 재시도가 원래 수량을 다시 팔던 문제, 종목표에 없는 보유를 정리할 길이 없어 주문이 영영 막히던 문제, 재현이 오늘 계좌 잔고를 예산으로 써서 과거 판단과 달라지던 문제. 보통 3: 보류 재시도 오류 격리 없음, 일부만 팔린 매도를 성공으로 세고 남은 수량 방치, 계좌 조회 실패가 QbotError가 아니면 판단까지 멈춤. 낮음 2: 날짜 인자 pattern 누락(format만으로는 검증되지 않는다), 대조 정리의 명령 번호 미기록·주문 멈춤 사유 미표시 |
...
| S2 + S3 월말 판단과 실행 | B3, D2 | 모의 계좌에서 판단 다음 날 10종목 매수, 다음 달 교체. **공시 적재가 끝나야 판단에 후보가 생긴다** |
| S5 재시작 | D1 | 주문 전송 직후 강제 종료 → 재시작 → 중복 주문 0건 |
| S6 + S7 손실 한도와 정지 | E, C2 | 한도 도달 시 매수 거부, `/halt` 뒤 미체결 취소, 재시작 후 정지 유지 |
| S13 계좌 불일치 | D2 | 앱에서 직접 매매 → 저녁 대조 불일치 → 주문 멈춤 → `/reconcile_accept`. 가짜 계좌로는 통과(테스트 11개), 실계좌 확인은 S2·S3 뒤 |
...
D2에서 더해진 것: [[QBOT-MS-001#TradingService.execute]] 1단계 앞에 "정지·주문 멈춤이면 실행하지 않는다"를 넣고, 8단계를 "보류·오류·관문 거부가 하나라도 있으면 partial"로 고친다(done은 다시 실행할 수 없으므로 전부 거부된 판단을 done으로 닫으면 그 달이 사라진다). 5단계의 보류는 "거래정지"뿐 아니라 "일부만 체결된 매도의 남은 수량"도 포함하고, 같은 판단에서 다시 팔 수 있게 멱등 키에 시도 번호를 붙인다. [[QBOT-MS-001]]에 TradingService.reconcile의 판정 기준을 적는다: 보유 수량 차이 또는 "마지막으로 맞은 대조의 계좌 현금 + 그 뒤 우리 체결"로 설명되지 않는 예수금 차이(허용 오차 max(1만원, 총평가액 1%)). [[QBOT-DOM-003#reconciliation]]에 `last_fill_id` 추가. [[QBOT-API-001]]에 명령줄 전용 `decide`·`execute`를 추가하고(스케줄 전까지 손으로 돌리는 입구), 날짜 인자에는 `pattern`을 `format`과 함께 둔다. [[QBOT-DOM-003#strategy_config]]에 지수 ETF 종목 코드 칸이 없어 코드 상수(069500)로 뒀다 — 칸 추가 검토. [[QBOT-DOM-003#bot_state]]에 주문 멈춤 사유 칸이 없어 `status`가 최근 불일치 대조를 대신 보여 준다.
