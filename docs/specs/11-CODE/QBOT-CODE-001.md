...
2026-09-22 기준 A~F 전 슬라이스의 코드가 있다. 남은 것은 코드가 아니라 운용이다 — 모의 3개월, 그 뒤 관문.
...
| 구현 함수 | LiveGate.check · [[QBOT-MS-001#RiskService.gate_approve]] · seed_live_db · refuse_live_without_gate · DecisionService.ideal_return |
...
| 완료 | main `0596533` (2026-09-22). 테스트 22개 추가(전체 197 통과). `risk/gate.py`가 다섯 조건(모의 90일·교체 3회·주문 오류 0·선정 일치 100%·월 수익 차이 1%p)을 집계해 GateRecord로 남긴다. **못 재는 조건은 "통과"가 아니라 "미달"이다** — 비교할 자료가 없으면 -1로 기록하고 막는다. 관문이 막은 주문은 오류로 세지 않는다(봇이 제대로 일한 것이다). 막는 층이 둘이다: 실전 모드인데 통과 기록이 없으면 서비스 조립 **전에** 시작을 거부하고, 뚫려도 OrderGate가 `no_gate`로 거부한다. 승인은 통과 기록 + 실전 키 + **실전 계좌 접속 시험**을 다 통과해야 한다. **리뷰가 잡은 치명적 설계 오류**: 모의와 실전은 DB 파일이 다른데(INFRA 6장) 승인을 모의 DB에만 남겨, 실전으로 시작하면 빈 파일을 열고 "관문 기록 없음"으로 영영 거부됐다 → 승인이 실전 DB를 만들어 기록과 상태를 옮겨 심는다(alembic으로 스키마까지). 리뷰 11건 반영: 재현 행이 모의 운용 기간을 늘려 2019년 날짜로 한 번 재현하면 90일 조건이 통과하던 문제, 선정 일치율이 봇을 봇 자신과 비교하던 문제(검증 파일 우선, 없는 달만 재현 가능성), 접속 시험이 모의 계좌를 찌르던 문제, 승인이 돌고 있는 모의 운용의 상태를 바꿔 첫 달 상한에 막히게 하던 문제, 장부상 수익률이 지수 부분을 빼 체결 오차가 아니라 구성 차이를 재던 문제, 주문 오류 평생 누적, 관문 점검이 재현 행을 쌓던 문제 등. **승인 자체는 실행하지 않았다** — 모의 운용 3개월이 선행 조건이고 실전 키도 없다 |

## 2. 통합 테스트

| 시나리오 | 슬라이스 | 검증하는 것 |
|---|---|---|
| S11 과거 날짜 재현 | B3, C1 | 2025-05~2026-08 월말 20개 전부 검증 파일(`top60_by_month.csv`)과 일치. **대기 중**: 일봉 574만 행·지수·상태·달력(2026-11-06까지)은 끝. 공시는 2026-09-22 16:48 차단이 풀려 이어받는 중(2023-08까지). 하루 한도 2만 건이라 며칠 걸린다 |
...
| S8 실전 전환 | F | 관문 미달 거부, 통과 후 승인, 첫 달 매수 상한. 가짜 계좌로는 통과(실전 DB 생성까지 확인), 실제 전환은 모의 3개월 뒤 |
...
E: [[QBOT-MS-001#RiskService.record_valuation]] 4단계를 "고점·원금을 입출금만큼 옮긴다"로 고친다(현재 문구대로면 입금 뒤 손실 한도가 무력화된다). 2단계의 "입출금 감지"는 "사람이 대조에서 밝힌 금액"으로 확정하고(3장 미결 해소) [[QBOT-DOM-003#reconciliation]]에 `external_flow`를 추가한다. 같은 날 두 번 기록해도 안전해야 한다는 조건을 넣는다. [[QBOT-API-001#reconcile_accept]]에 `external_flow` 인자 추가. [[QBOT-API-001]]에 `run`(상주 프로세스, 명령줄 전용) 추가. [[QBOT-INFRA-001]] 8.1의 "매시 생존 신호"는 "기록은 매시, 메시지는 하루 한 번". 8.2 자동 시작은 systemd 사용자 유닛으로 확정(9장 미결 해소). 3장에 "SQLite는 BEGIN IMMEDIATE + busy_timeout으로 쓴다"를 추가. [[QBOT-DOM-002]] 4.6 ReportingService의 `factor_effects`는 월간 보고에서 빼고 연 1회 재점검에만 둔다. [[QBOT-DOM-003#bot_state]]에 주문 멈춤 사유 칸이 없어 `status`가 최근 불일치 대조를 대신 보여 준다.

F: [[QBOT-MS-001#RiskService.gate_approve]] 5단계를 "모의 상태는 그대로 두고 실전 데이터베이스를 새로 만들어 통과 기록과 상태를 심는다"로 고친다. 지금 문구(`state.mode = live`)대로 하면 모의 DB의 상태만 바뀌어 실전은 빈 파일로 시작하고, 재시작 전까지 돌고 있는 모의 운용이 첫 달 상한에 막힌다. [[QBOT-PRD-001#R8]]의 "선정 일치율 검증 코드와 100%"는 검증 파일이 덮는 기간(2024-12~2026-08)에만 잴 수 있다 — 그 밖의 달은 "그날 저장된 판단과의 재현 일치"로 갈음한다고 적어야 한다. "주문 오류 0건"은 평가 구간(직전 90일) 안에서 센다고 명시(평생 누적이면 석 달 전 일시 오류 하나가 영영 막는다). "월 수익 차이"의 비교 대상은 종목·지수를 비중대로 섞은 장부상 수익률이다. [[QBOT-DOM-003#gate_record]]에 "못 쟀음"을 -1로 남긴다고 적는다. [[QBOT-API-001#gate_check]]는 명령줄 전용이다(월마다 재현을 돌려 몇 분 걸린다).
...
- [ ] 재점검·관문의 시가총액은 현재 주식 수로 계산한다(과거 주식 수 미보관). 과거 월말 EP·SP·TURN20에 영향. 상장주식수 이력을 남길지 결정 필요
- [ ] 실전 첫 달 뒤 "체결 오차 1%p 이내면 나머지 투입"([[QBOT-PRD-001#R8]] 마지막 줄)은 아직 코드가 없다. 첫 달 상한을 푸는 도구가 필요하다
