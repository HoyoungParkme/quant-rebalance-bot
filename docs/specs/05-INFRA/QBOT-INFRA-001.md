...
2026-09-22에 실제로 돌려 보고 확인된 수치와 제약, 2026-09-23 모의 계좌 실주문에서 확인된 모의투자의 제약, 같은 날 저녁 수집에서 확인된 당일 봉의 잠정성을 반영했다.
...
예외는 하나다. **아직 확정되지 않은 봉**은 확정값으로 고친다. 봉은 그 거래일보다 **뒤에** 받아야 확정이다. 증권사 당일 봉은 장이 끝난 뒤에도 저녁까지 바뀐다 — 2026-09-23 같은 종목의 당일 종가가 18:30과 19:44에 달랐고, 전날 18:58에 받은 종가는 다음 날 받은 값과 802종목에서 달랐다. 그래서:

- 같은 날 받은 봉은 잠정값이다. 다음 수집 때 새로 받은 값으로 덮고 받은 시각도 바꾼다(그러면 확정이 된다)
- 수정주가 판정(새 판)은 **확정 봉끼리만** 비교한다. 잠정 봉을 비교에 쓰면 전 종목을 수정주가로 오판해 이력을 통째로 다시 받는다(2026-09-22 장중 값, 2026-09-23 저녁 값 — 두 번 겪었다)
- 지수도 같다. 저녁에 받은 지수는 다음 수집·아침 확정에서 덮는다
...
- 일봉 조회는 한 페이지 100개, **지수 일별 조회는 50개**다. 페이지 크기를 가정하지 않는다(가정했다가 지수 이력을 두 달치만 받았다)
...
| 연구용 자료(일괄 재무 파일, 확장 재무, 모델) | `~/.qbot/research-data/` | 올리지 않는다(결과 표만 `docs/research/`) | 수동 |

- 데이터 폴더는 홈 아래 `~/.qbot/`이다. 모의용과 실전용 데이터베이스 파일을 따로 둔다. **실전으로 넘어갈 때 관문 기록을 실전 파일에도 심어야 한다** — 안 그러면 빈 파일을 열고 "관문 없음"으로 거부된다
- 백업은 `VACUUM INTO`로 만든다. WAL이라 파일 복사는 반쪽이 될 수 있고, SQLite 백업 API는 적재가 쓰는 동안 복사를 계속 처음부터 다시 시작한다. `VACUUM INTO`는 읽기 스냅숏 하나로 끝난다. 최근 30일치를 남긴다(한 벌 약 800MB)
...
| 변경 | 영향 | 대응 |
|---|---|---|
| 액면분할, 무상증자, 감자 | 그 종목의 과거 주가가 통째로 달라진다 | 수정주가를 받는다. **확정 봉**의 저장 종가가 새 응답과 다르면 새 판으로 과거를 다시 받아 추가한다. 옛 판은 남긴다. 잠정 봉(같은 날 받은 봉)은 비교에 쓰지 않는다([[#C10]]). 2026-09-23 수정 뒤 첫 수집에서 잡힌 것은 1종목(주식 병합, 과거 가격 전부 ×5)뿐이었다 |
...
| 07:00 (전 거래일이 월말일 때만) | 월말 봉·지수를 **확정값으로 다시 받고** 월말 판단. 기준일은 월말 그대로 | [[QBOT-UC-001#UC-A2]] |
...
| 18:30 | 시세·공시 수집(당일 봉은 잠정), 계좌 대조, 평가액, 손실 한도, 일일 요약 | [[QBOT-UC-001#UC-A1]], [[QBOT-UC-001#UC-A5]] |
...
**월말 판단을 다음 날 아침에 하는 이유.** 월말 당일 저녁의 종가는 잠정값이다([[#C10]]). 그걸로 종목을 고르면 나중에 같은 날짜를 재현했을 때(확정 종가) 다른 종목이 나오고, 실전 전환 관문의 "선정 일치 100%"를 영영 채우지 못한다. 기준일(월말)과 공시 범위(기준일 전날까지)는 그대로라 입력은 저녁 판단과 같고, 가격만 확정값이 된다. 대가는 운영자가 판단을 보고 막을 시간이 밤새에서 07:00~08:40으로 줄어든 것이다. 확정에 실패한 종목이 있으면 잠정값으로 판단하고 알림을 보낸다.
...
| 절전 | 끈다. 07:00 월말 판단이 있으므로 아침에도 켜져 있어야 한다 |
| 운영체제 자동 업데이트 재부팅 | 사용 시간을 07시~20시로 잡아 그 사이에 재부팅되지 않게 한다 |
| 시계 | 인터넷 시간 동기화를 켠다 |
...
- [ ] 당일 봉이 **몇 시에** 확정되는지. 2026-09-23 20:05·20:35·21:30과 다음 날 값을 비교해 확인 중(`~/.qbot/research-data/close-probe/`). 저녁 수집 시각을 옮길지 결정 근거
