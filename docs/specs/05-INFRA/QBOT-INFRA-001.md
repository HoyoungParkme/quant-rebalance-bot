...
2026-09-22에 실제로 돌려 보고 확인된 수치와 제약을 반영했다.
...
- 백업은 SQLite 백업 API로 복사한다. WAL이라 파일 복사는 반쪽이 될 수 있다. 최근 30일치를 남긴다
...
| 변경 | 영향 | 대응 |
|---|---|---|
| 액면분할, 무상증자, 감자 | 그 종목의 과거 주가가 통째로 달라진다 | 수정주가를 받는다. 어제 종가가 저장값과 다르면 새 판으로 과거를 다시 받아 추가한다. 옛 판은 남긴다 |
